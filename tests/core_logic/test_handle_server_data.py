...
class TestSetAndValidateCoordinate(TestCase):
    ...
    def test_handle_atom_data(self):
        for data, validated in self.atom_data:
            ...
            if validated == 'valid':
                atom_logic.handle_server_data(data)
                self.assertTrue(atom_logic.is_it_atom())
            else:
                atom_logic.handle_server_data(data)
                self.assertFalse(atom_logic.is_it_atom())
